...
try {
    ...
    if ($autoProcess && !empty($url)) {
        try {
            ...
            // デバッグ用
            error_log("article_detail.php (autoProcess): 記事本文を取得しました。長さ: " . strlen($body));
    ...
    if ($autoFetchArticle && !empty($url)) {
        try {
            ...
            // デバッグ用
            error_log("article_detail.php (autoFetchArticle): 記事本文を取得しました。長さ: " . strlen($body));
            ...
            if (!empty($articleContent)) {
                ...
                if ($autoImprove) {
                    ...
                    $successMessage .= ' さらに、記事を分析して改善しました。';
                ...
                throw new Exception('記事の内容を取得できませんでした。');
            }
        } catch (Exception $e) {
    ...
    // 記事の改善処理（フォーム送信時）
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['improve_article'])) {
        try {
            // フォームから記事内容を取得
            $articleContent = $_POST['article_content'] ?? '';
            
            if (empty($articleContent)) {
                throw new Exception('記事内容が空です。');
            }
...
<body>
    <nav class="navbar navbar-expand-lg navbar-dark bg-primary">
        <div class="container">
            <a class="navbar-brand" href="index.php">記事改善管理システム</a>
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav">
                    ...
                    <li class="nav-item">
                        <a class="nav-link" href="batch_process.php">バッチ処理</a>
                    </li>
                </ul>
            </div>
        </div>
